...
*   Execute this command in ChimeraX:
    ```bash
    run "/path_to_project/SymProFold/assemblies/Vaer_combinatorial_search.py"
    ```
...
The results of the combinatorial search are collected in the folder `/path_to_project/SymProFold/assemblies/Vaer_Vibrio aerogenes_A0A1M5ZCF8_v[version]/`.
Each subfolder represents one symmetry complex combination, the quality score can be read from the subfolder name.

For easier evaluation, the results are also sorted into the folders `ranking_candidates/` and `filtered_out/`.  
The folder `ranking_candidates/` contains filenames representing completed runs with `score_clash < 2`.  
The folder `filtered_out/` contains incomplete runs (termination because of axis tilts >45°, too large distances between the symmetry complexes to be connected etc.) and runs with a very high clash score (`score_clash ≥ 2`), indicating significant domain overlap.
```
assemblies/Vaer_Vibrio aerogenes_A0A1M5ZCF8_v[version]/
├── ...
├── 44_23-FL_.073_.063_.010_.0052_d2_1_1-4/
├── ...
├── filtered_out/
├── ranking_candidates/
└── ...
...
naming scheme:
[  4  ][  4  ]_[   23    ]-[   FL    ]_[   .073   ]_[  .063  ]_[  0.010   ]_[…]_d[   2   ]_[…]_[  1   ][  4   ]
[symm0][symm1]_[subchain0]-[subchain1]_[score_qual]_[score_cl]_[score_bend]_[…]_d[aln_dom]_[…]_[model0][model1]

[symm0]:        order of rot. symmmetry axis 0 (symmetry complex 0)
[symm1]:        order of rot. symmmetry axis 1 (symmetry complex 1)
...
[aln_dom]:      alignment domain
[model0]:       model number (starting with 0) for symmetry complex 0
[model1]:       model number (starting with 1) for symmetry complex 1
```
*   Identify the symmetry complex combination with the best (smallest) quality score (score_quality). In this tutorial, the score is minimal for `44_23-FL_.073_.063_.010_.0052_d2_1_1-4/`.
...
A lookup in folder `snapshot1_ax_predictions/` shows an ipTM+pTM score of 0.713 for symmetry complex 0 (model 1) and an score of 0.567 for symmetry complex 1 (model 4).

#### 5.1.3 Consideration of quality score and ipTM+pTM

In principle, we now can use the two identified models (symmetry complex 0 and 1) as input for the assembly script (section 5.2). However, in this example, the assembly model can further be optimized by replacing one of both models (symmetry complexes) by a comparable model with higher ipTM+pTM score.

By comparing the assembly models, we find that the symmetry complex 1 model can be replaced while retaining the assembly and lattice constant (a = 12.7nm):  
symmetry complex 0: subchain 23, model 1: 0.713 [ipTM+pTM]  
symmetry complex 1: subchain 12, model 0 (instead of FL, model 4): 0.873 [ipTM+pTM]
